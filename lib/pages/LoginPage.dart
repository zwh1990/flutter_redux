...
class _LoginPageState extends State<LoginPage> {
  ...
  void login(String name, String password) async {
    ...
    if (data.code == "0") {
      //把账号密码保存
      saveUserAndPassword(name, password);
      Navigator.push<String>(
          context, new MaterialPageRoute(builder: (context) => new HomePage()));
    }
  }

  void saveUserAndPassword(String name, String pass) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setString(Constant.NAME, name);
    prefs.setString(Constant.PASSWORD, pass);
  }
